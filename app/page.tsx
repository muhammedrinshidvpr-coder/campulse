...
function AppleButton({
  ...
  return (
    <button
      className={cn(
        "relative inline-flex items-center justify-center font-medium transition-all active:scale-[0.97] disabled:opacity-60 disabled:pointer-events-none",
        sizeClasses[size],
        variants[variant],
        className,
      )}
    ...
    <div className="w-full">
      <div className="mb-2 flex items-end justify-between text-sm font-medium tracking-tight">
        <span className="flex items-center gap-1.5 text-slate-900">
          <Zap className="h-4 w-4 fill-sky-500 text-sky-500" /> Level {level}
        </span>
            ...
            <div
              className={cn(
                ...
                isCompleted
                  ...
                  : isCurrent
                    ? "border-2 border-sky-600 bg-white shadow-[0_0_0_4px_rgba(2,132,199,0.1)]"
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === "signin") {
        ...
        if (!data.session) {
          setInfo(
            "Account created. Check your email (including spam) for a confirmation link, then sign in.",
          );
          setMode("signin");
        }
      }
      ...
      if (isAuthApiError(err) && err.code === "email_not_confirmed") {
        setError(
          "This email hasn't been confirmed yet. Check your inbox for the confirmation link, or resend it below.",
        );
        ...
        setError(
          "Too many emails sent to this address recently. Wait a bit and try again.",
        );
      } else {
  ...
  return (
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-slate-50 p-4">
      ...
      <div className="relative z-10 flex w-full max-w-md flex-col items-center rounded-[28px] border border-slate-200/70 bg-white p-8 shadow-[0_8px_40px_rgba(0,0,0,0.04)] sm:p-10">
        ...
        <p className="mb-8 text-center text-sm tracking-tight text-slate-500">
          See the change. Shape the campus.
        </p>
        <form className="w-full space-y-4" onSubmit={handleSubmit}>
          {mode === "signup" && (
            <input
              ...
              className="w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3.5 text-sm text-slate-900 transition-all placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-sky-200"
            />
          )}
          <input
            type="email"
          ...
          {needsConfirmation && (
            <button
              ...
              disabled={resending}
              className="w-full text-center text-xs font-medium text-sky-700 hover:underline disabled:opacity-60"
            >
              {resending ? "Resending…" : "Resend confirmation email"}
            </button>
          )}
          {info && (
            <p className="rounded-xl bg-sky-50 px-4 py-2.5 text-xs font-medium text-sky-700">
              {info}
            </p>
          )}
          <AppleButton
        ...
        <div className="mt-8 w-full text-center">
          <p className="flex items-center justify-center gap-1.5 text-xs font-medium text-slate-500">
            <Shield className="h-3.5 w-3.5" /> Your campus. Your voice.
            Protected.
          </p>
        </div>
      </div>
...
function HomeScreen({
  ...
  useEffect(() => {
    ...
    const runCheck = async () => {
      if (!supabase) {
        ...
        setSupabaseMessage(
          "Supabase credentials are missing. Add your URL and anon key to the environment file.",
        );
        return;
      }
      ...
      try {
        ...
        if (error) {
          setSupabaseStatus("error");
          setSupabaseMessage(
            `Connected, but the schema isn't ready: ${error.message}`,
          );
        } else {
  ...
  useEffect(() => {
    ...
    fetchMyIssues(profileId)
      .then((rows) => setRecentIssues(rows.slice(0, 2)))
      .catch(() => {});
  }, [profileId]);
  ...
  const { level, currentLevelXp, xpForNextLevel } = levelFromScore(
  ...
  return (
    <div className="space-y-6 pb-20 md:pb-6">
      <AppleCard
        className={cn(
          "border-slate-200/70",
          supabaseStatus === "connected" ? "bg-emerald-50/70" : "bg-slate-50",
        )}
      >
        <div className="flex items-start gap-3">
          <div
      ...
      <div className="grid grid-cols-2 gap-4">
        <button
          ...
          className="flex flex-col items-start gap-4 rounded-2xl bg-sky-600 p-5 text-white shadow-sm transition-transform active:scale-95"
        >
          <div className="rounded-full bg-white/20 p-2.5">
            <AlertCircle className="h-5 w-5" />
          </div>
          <span className="font-medium tracking-tight">Report</span>
        </button>
        <button
          onClick={() => onNavigate("feed")}
          className="flex flex-col items-start gap-4 rounded-2xl border border-slate-200 bg-white p-5 text-slate-900 shadow-sm transition-transform active:scale-95"
        >
          <div className="rounded-full bg-sky-100 p-2.5 text-sky-700">
            <MessageSquare className="h-5 w-5" />
          </div>
          <span className="font-medium tracking-tight">Feed</span>
        </button>
      </div>
      <AppleCard className="border-none bg-gradient-to-br from-sky-600 to-indigo-600 text-white">
        <div className="mb-6 flex items-start justify-between">
          <h3 className="flex items-center gap-2 text-sm font-medium text-white/90">
            <BarChart3 className="h-4 w-4" /> Pulse Widget
          </h3>
        ...
        <div className="flex items-end gap-6">
          <div>
            <div className="text-4xl font-semibold tracking-tighter">
              {stats?.resolution_rate_pct != null
        ...
        <div className="space-y-3">
          {recentIssues.length === 0 ? (
            <div className="rounded-2xl border border-dashed border-slate-200 bg-white p-6 text-center text-sm text-slate-500">
              No reports yet. Your first one earns a badge.
            </div>
          ) : (
            recentIssues.map((issue) => (
              <button
                key={issue.id}
                onClick={() => onNavigate("my-reports")}
                className="flex w-full items-center gap-4 rounded-2xl border border-slate-200 bg-white p-4 text-left shadow-sm"
              >
                <div
                  className={cn(
                ...
                <div className="flex-1">
                  ...
                  <p className="mt-0.5 text-xs text-slate-500">
                    {timeAgo(issue.updated_at)} • {STATUS_LABELS[issue.status]}
                  </p>
                </div>
  ...
  if (result) {
    return (
      <div className="flex min-h-[60vh] flex-col items-center justify-center space-y-6 text-center">
        <div className="mb-2 flex h-24 w-24 items-center justify-center rounded-full bg-emerald-100 text-emerald-600">
        ...
        <div>
          ...
          <p className="text-sm font-medium text-slate-500">
            Tracking ID:{" "}
            <span className="text-sky-600">#{result.tracking_code}</span>
          </p>
        </div>
        <AppleCard className="w-full p-6 text-left">
          <h3 className="mb-4 text-sm font-semibold tracking-tight text-slate-900">
            Quest Progress
          </h3>
          <QuestStepper
            steps={STATUS_STEPS.map((s) => STATUS_LABELS[s])}
            currentStep={STATUS_STEPS.indexOf(result.status)}
          />
        </AppleCard>
        <AppleButton
          onClick={() => onNavigate("my-reports")}
    ...
    <div className="space-y-6 pb-20 md:pb-6">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-500">
          Takes &lt;60s. Your voice shapes campus.
        </p>
      </div>
      <div className="space-y-5">
        <div className="space-y-1.5">
          <label className="ml-1 text-sm font-medium tracking-tight text-slate-900">
            Category
          </label>
          <select
            value={category}
            onChange={(e) => setCategory(e.target.value as IssueCategory)}
            className="w-full appearance-none rounded-2xl border border-slate-200 bg-white px-4 py-3.5 text-sm text-slate-900 shadow-sm focus:outline-none focus:ring-2 focus:ring-sky-200"
          >
            {CATEGORIES.map((c) => (
        ...
        <div className="space-y-1.5">
          ...
          <div className="relative">
            ...
            <input
              ...
              placeholder="e.g. North Library, Floor 2"
              className="w-full rounded-2xl border border-slate-200 bg-white pl-11 pr-4 py-3.5 text-sm text-slate-900 shadow-sm focus:outline-none focus:ring-2 focus:ring-sky-200"
            />
          </div>
        </div>
        <div className="space-y-1.5">
        ...
        <div className="space-y-1.5">
          ...
          <textarea
            ...
            className="w-full resize-none rounded-2xl border border-slate-200 bg-white px-4 py-3.5 text-sm text-slate-900 shadow-sm focus:outline-none focus:ring-2 focus:ring-sky-200"
          />
        </div>
        <input
          id="photo-input"
        ...
        <label
          htmlFor="photo-input"
          className="flex w-full cursor-pointer flex-col items-center justify-center gap-2 rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-5 text-sky-700 transition-colors hover:bg-sky-50"
        >
        ...
        <AppleCard className="flex items-center justify-between p-4 sm:p-4">
          ...
          <button
            ...
            className={cn(
              "flex h-7 w-12 items-center rounded-full p-0.5 transition-colors duration-300",
              isAnon ? "bg-emerald-500" : "bg-slate-400",
            )}
          >
            <div
              className={cn(
  ...
  return (
    <div className="space-y-6 pb-20 md:pb-6">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-500">
          Every report you&apos;ve filed, tracked end to end.
        </p>
      </div>
      {issues.length === 0 ? (
        ...
        <div className="space-y-4">
          {issues.map((issue) => (
            <AppleCard key={issue.id}>
              ...
              <QuestStepper
                steps={STATUS_STEPS.map((s) => STATUS_LABELS[s])}
                currentStep={STATUS_STEPS.indexOf(issue.status)}
              />
...
function FeedScreen() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const [ann, issues] = await Promise.all([
          fetchAnnouncements(),
          fetchPublicIssues(),
        ]);
  ...
  return (
    <div className="space-y-6 pb-20 md:pb-6">
      <div>
        ...
        <p className="text-sm text-slate-500">The campus, in numbers.</p>
      </div>
      {loading ? (
        <div className="py-20 text-center text-sm text-slate-500">
          Crunching campus numbers…
        </div>
      ) : (
        <>
          <div className="grid grid-cols-2 gap-4">
            <AppleCard className="aspect-square p-5">
            ...
            <AppleCard className="aspect-square p-5">
              ...
              <div className="text-4xl font-semibold tracking-tighter text-slate-900">
                {avgResponseDays}
                <span className="ml-0.5 text-xl text-slate-500">d</span>
              </div>
            </AppleCard>
            <AppleCard className="aspect-square border-none bg-slate-100 p-5">
              <div className="mb-2 text-xs font-semibold uppercase tracking-wider text-rose-500">
                Top Category
              </div>
              <div className="mt-auto text-2xl font-semibold leading-none tracking-tight text-slate-900">
                {topCategory}
              </div>
            </AppleCard>
          </div>
          <AppleCard>
            <h3 className="mb-4 flex items-center gap-2 text-base font-semibold tracking-tight text-slate-900">
              <TrendingUp className="h-4 w-4 text-sky-700" /> Trending Patterns
            </h3>
            {trendingLocations.length === 0 ? (
              <p className="text-sm text-slate-500">
                No repeated locations yet — good sign.
              </p>
            ) : (
              <div className="space-y-4">
                {trendingLocations.map(([location, count], idx) => (
                  <div
                    ...
                    className={cn(
                      ...
                      idx < trendingLocations.length - 1 &&
          ...
          <div className="py-8 text-center">
            <Shield className="mx-auto mb-3 h-8 w-8 text-slate-400 opacity-50" />
            <p className="text-xs font-medium uppercase tracking-wider text-slate-500">
              Every report is tracked.
              <br />
              Every resolution is visible.
            </p>
          </div>
        </>
      )}
...
function RewardsScreen({ profile }: { profile: Profile | null }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const [allBadges, lb] = await Promise.all([
          fetchAllBadges(),
          fetchLeaderboard(),
        ]);
        setBadges(allBadges);
  ...
  return (
    <div className="space-y-6 pb-20 md:pb-6">
      <div className="relative overflow-hidden rounded-3xl border border-slate-200 bg-white p-8 text-center shadow-sm">
        <div className="pointer-events-none absolute right-[-10%] top-[-20%] rotate-12 opacity-5">
        ...
        <div className="relative z-10">
          ...
          <h2 className="mb-4 text-6xl font-semibold tracking-tighter text-slate-900">
            {profile?.contribution_score ?? 0}
            <span className="ml-1 text-2xl text-slate-500">XP</span>
          </h2>
          <div className="inline-flex items-center gap-2 rounded-full bg-slate-100 px-4 py-2">
            <ShieldAlert className="h-4 w-4 text-amber-500" />
            <span className="text-sm font-semibold tracking-tight text-slate-900">
              Campus Voice Lvl {level}
            </span>
          </div>
        </div>
      </div>
            ...
            {badges.map((badge) => {
              ...
              return (
                ...
                >
                  <div
                    className={cn(
                      "flex h-20 w-20 items-center justify-center rounded-2xl border bg-white transition-transform hover:scale-105",
                      owned
                        ? "border-sky-200 shadow-[0_4px_20px_rgba(2,132,199,0.15)]"
...
export default function HomePage() {
  ...
  return (
    <div className="flex min-h-screen flex-col bg-slate-50 text-slate-900 md:flex-row">
      <header className="sticky top-0 z-40 border-b border-slate-200 bg-slate-50/80 p-4 backdrop-blur-xl md:hidden">
        <div className="flex items-center justify-between">
      ...
      {isMobileMenuOpen && (
        <div className="fixed inset-x-0 top-[60px] z-30 border-b border-slate-200 bg-white p-4 shadow-sm md:hidden">
          <p className="mb-3 truncate text-xs font-medium text-slate-400">
            Signed in as {profile?.full_name || firstName}
          </p>
          <button
      ...
      <aside className="fixed left-0 top-0 hidden h-screen w-64 flex-col border-r border-slate-200 bg-white text-slate-900 md:flex">
        ...
        <div className="mt-auto p-6">
          ...
          <button
            className="flex w-full items-center gap-3 text-left text-sm font-medium text-slate-500 transition-colors hover:text-slate-900"
      ...
      <main className="mx-auto flex-1 w-full max-w-3xl p-4 md:ml-64 md:p-8">
        {renderContent()}
      </main>
      <nav className="fixed bottom-0 left-0 right-0 z-40 border-t border-slate-200 bg-white/80 pb-safe backdrop-blur-xl md:hidden">
          ...
          {navItems.map((item) => {
            ...
            return (
              <button
                ...
                onClick={() => setCurrentTab(item.id)}
                className={cn(
                  "relative flex h-14 w-16 flex-col items-center justify-center transition-colors",
              ...
              >
                {item.special ? (
                  <div
                    className={cn(
                      "absolute -top-5 flex h-12 w-12 items-center justify-center rounded-full shadow-md transition-transform",
                      isActive
                        ? "bg-sky-600 text-white -translate-y-1"
                        : "border border-slate-200 bg-white text-sky-700 hover:bg-slate-100",
                    )}
                  >
      ...
      {toastMessage ? (
        <div className="fixed bottom-24 left-1/2 z-50 flex -translate-x-1/2 items-center gap-3 rounded-full bg-slate-900 px-5 py-3.5 text-white shadow-lg">
